...
var (
	IoctlMEIConnectClient = IOWR('H', 0x01, uintptr(len(ClientGUID{})))
)
...
func (m *MEI) Open(meiPath string, guid ClientGUID) error {
	...
	if _, _, err := syscall.Syscall(syscall.SYS_IOCTL, fd.Fd(), IoctlMEIConnectClient, uintptr(unsafe.Pointer(&data))); err != 0 {
		return fmt.Errorf("ioctl IOCTL_MEI_CONNECT_CLIENT failed: %v", err)
	}
...
func (m *MEI) Write(p []byte) (int, error) {
	...
	return syscall.Write(int(m.fd.Fd()), p)
}
...
func (m *MEI) Read(p []byte) (int, error) {
	...
	return syscall.Read(int(m.fd.Fd()), p)
}
